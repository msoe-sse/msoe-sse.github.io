...
var GitHubActivity = (function() {
  ...
  var methods = {
    renderLink: function(url, title, cssClass) {
      ...
      return Mustache.render(
        '<a class="' + cssClass + '" href="{{url}}" target="_blank">{{{title}}}</a>',
        { url: url, title: title }
      );
    },
    ...
    getMessageFor: function(data) {
      ...
      if (p.ref) {
        ...
        data.branchLink =
          methods.renderGitHubLink(data.repo.name + "/tree/" + data.branch, data.branch) + " at ";
      }
    ...
    getHeaderHTML: function(data, repo) {
      if (data.name && !!repo) {
        let url = data.html_url + "/" + repo;
        data.repoNameLink = methods.renderLink(url, repo);
        data.repo = repo;
      } else {
        data.withoutName = " without-name";
      }
      console.log(data);
      data.userLink = methods.renderLink(data.html_url, data.name);
      data.gravatarLink = methods.renderLink(data.html_url, '<img src="' + data.avatar_url + '">');

      return Mustache.render(templates.RepoHeader, data);
    },
    getActivityHTML: function(data, limit) {
      var text = `<div class="gha-activities">`;
  ...
  obj.feed = function(options) {
    ...
    if (!!options.repository) {
      eventsUrl =
        "https://api.github.com/repos/" + options.username + "/" + options.repository + "/events";
    }
    ...
    methods.getOutputFromRequest(userUrl, function(error, output) {
      if (error) {
        ...
        header = methods.getHeaderHTML(output, options.repository);
      }
...
var templates = {
    ...
    RepoHeader:
      '<div class="gha-header">\
                  ...
                  <div class="gha-user-info{{withoutName}}">{{{repoNameLink}}}<p>{{{userLink}}}</p></div>\
